...
<section class="gallery_page">
    <div class="container">
        ...
        <div class="photo_gallery__filter" id="archive_browser">
            <div class="gallery_filter__title">
                <h3 class="title">Фотозвіт заходів</h3>
            </div>
            <div class="gallery_filter__select">
                <p>оберіть рік</p>
                <select id="month-choice">
                    <option value="no-choice" class="select_year">Всi роки</option>
        ...
        <div class="photo_gallery__box">
            <div class="row" id="archive_pot">
                <?php $galleryImageArray = array( 'post_type' => 'gallery', 'numberposts' => -1, 'order' => 'DESC', 'tax_query' => array(
                    array(
    ...
    (function ($) {
        ...
        $(function () {
            ...
            $("#archive_browser select").change(function () {
                $(".gallery_social").hide();
                $("#archive_pot").empty().html(
                    "<div style='text-align:center; padding:200px;margin: 0 auto;'><img src='<?php bloginfo('template_url') ?>/assets/css/ajax-loader.gif' /></div>"
                );;
                var dateArray = $("#month-choice").val().split("/");
                var y = dateArray[3];
                $.post(
                    "<?php bloginfo('template_url') ?>/template-parts/archive_getter.php", {
                        year: y,
                    },
                    function (data) {
                        $("#archive_pot").hide().html(data).fadeIn('slow');
                        $(".gallery_social").fadeIn('slow');
                        loadGallery(16);
                        var gallery = $('.gallery_load');
                        if(gallery.length > 16){
