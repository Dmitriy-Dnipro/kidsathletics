...
<?php 
    $event_banner_image = get_field("event_banner_image", $post_id);
...
<style>
    .event_page__banner {
        background-image: url("<?php echo $event_banner_image['url'];?>");
    }
</style>

<section class="event_page__banner page_banner">
    <div class="container">
        <h2>
            <?php the_title();?>
        </h2>
...
<section class="event_page__information">
    <div class="container">
        ...
        <div class="event_info__descr">
            <h4>
                <?php echo get_field("event_info_title", $post_id);?>
            </h4>
            <p>
                <?php echo get_field("event_info_text", $post_id);?>
            </p>
        </div>
    </div>
</section>

<section class="event_page__step">
    <div class="container">
        <h4>« <?php the_title();?> »</h4>
        <div class="line_box">
            <div class="line red"></div>
        ...
        <div class="event_steps">
            <div class="event_step">
                <h5><span>1.</span> учасники</h5>
                <p>
                    <?php echo get_field("event_info_partic", $post_id);?>
                </p>
                <a href="#event_school" class="school">всі учасники</a>
            </div>
            <div class="event_step">
                <h5><span>4.</span> контакти</h5>
                <p><span>Адреса:</span>
                    <?php echo get_field("event_info_adress", $post_id);?>
                </p>
                <p><span>Сайт:</span> <a href="http://<?php echo get_field(" event_info_site", $post_id);?>">
                        <?php echo get_field("event_info_site", $post_id);?></a></p>
                <p><span>Тел:</span> <a href="tel:<?php echo get_field(" event_info_phone", $post_id);?>">
                        <?php echo get_field("event_info_phone", $post_id);?></a></p>
            </div>
            <div class="event_step">
                <h5><span>2.</span> місце проведення</h5>
                <p>
                    <?php echo get_field("event_info_where", $post_id);?>
                </p>
                <h6>Початок: <span>
                        <?php echo get_field("event_info_time", $post_id);?></span></h6>
            </div>
            <div class="event_step">
                <h5><span>3.</span> організатор</h5>
                <p>
                    <?php echo get_field("event_info_organizer", $post_id);?>
                </p>
            </div>
        </div>
    </div>
...
<section class="event_page__school" id="event_school">
        <div class="container">
            <h3 class="title">Всі учасники заходу:</h3>
            <ul class="school_box">
             <?php
                $school_event = get_field("school_event", $post_id);
                foreach($school_event as $school){?>
                <?php if(!empty($school['school'])){?>
                    <li><img src="<?php echo get_template_directory_uri();?>/images/notification.png"><?php echo $school['school'];?></li>
                 <?}?>
              <?}?>
            </ul>
        </div>
...
<section class="event_page__time">
    <div class="container">
        <h3 class="title">Регламент проведення заходу</h3>
        <div class="event_list__box">
            <ul class="event_tile__list">
                <?php 
                    $reg_events = get_field("event_reg", $post_id);
                    if(!empty($reg_events)){
                        for($i = 0; $i < 4; $i++){?>
                            <?php if(!empty($reg_events[$i])){?>
                                <li>
                                    <p><span><?php echo $reg_events[$i]['event_reg_start'];?></span> -
                                    <?php echo $reg_events[$i]['event_reg_events'];?>;</p>
                                </li>
                            <?}?>
                    <?}?>
                    <?}else{
...
<section class="event_page__photo">
    <div class="container">
        <h3 class="title">Фотозвіт попереднix заходiв</h3>
        <div class="line_box">
            <div class="line red"></div>
            <div class="line blue"></div>
            <div class="line green"></div>
        ...
        <div class="event_page__photo-box">
            <div class="row">
                <?php 
                    $event_gallery = get_field("event_photo_gallery", $post_id);

                    if($event_gallery){
                        foreach($event_gallery as $gallery){?>
                <div class="col-md-4">
                    <div class="event_page__photo-item">
                        <a href="<?php echo $gallery['url'];?>" data-fancybox="images">
                          <div class="event_item__photo">
                            <img src="<?php echo $gallery['url'];?>" alt="<?php echo $gallery['alt'];?>">
                          </div>
                        </a>
                        <!-- <div class="event__photo-item-border">
                            <p>Фотозвіт «Kids start»</p>
                        </div> -->
                    </div>
                </div>
                <?}?>
            ...
            <div class="event-page__photo-more">
                <h4>Більше фото</h4>
                <a href="#" class="more_photo">в галерею</a>
                <div class="photo-more__link">
                    ...
                    <a href="https://www.facebook.com/kidsathleticsdnepr/" target="_blank"><svg width="51" height="51" viewBox="0 0 51 51" fill="none" xmlns="http://www.w3.org/2000/svg">
...
<section class="partners_block">
    <div class="container">
        ...
        <?php
            $event_partners_gallery = get_field("event_partners_gallery", $post_id);
        ?>
                
             <div class="event_page__slider">
            <?php $countSlide = 0; ?> 
             <?php foreach($event_partners_gallery as $event_partners){
                   $countSlide++;
              ?>
                <?php if(!empty($event_partners)){?>
        ...
        <div class="event_page__nav">
            <?php 
             $count = 0;
            foreach($event_partners_gallery as $count_nav){
                $count++;
            ?>
